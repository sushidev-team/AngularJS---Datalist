...
angular.module('ambersive.datalist').run(['$templateCache', function($templateCache) {
  'use strict';

  $templateCache.put('src/views/datalist.default.html',
    "<div class=datalist_container><div class=datalist_header></div><div class=datalist_body><ul><li ng-if=\"data.length > 0\" class=header><div class=\"row header\"><div class=\"col-sm-8 col-xs-8\"><div class=checkbox><label><input type=checkbox ng-model=allSelected ng-click=\"itemCheckClickAll(allSelected,'id');\"></label></div></div><div class=\"col-sm-4 col-xs-4\"><div class=btn-group role=group><button type=button class=\"btn btn-default\" ng-disabled=actionDisabled ng-click=delete(selectedData);><i class=\"glyphicon glyphicon-remove\"></i></button></div></div></div></li><li ng-repeat=\"row in data\"><div class=row><div class=\"col-sm-8 col-xs-8\"><div class=checkbox><label><input type=checkbox ng-model=row.isSelected ng-click=insertItemClick($event,row.isSelected,row,identityValue);> {{ row[titleValue] }}</label></div></div><div class=\"col-sm-4 col-xs-4\"><div class=btn-group role=group><button type=button class=\"btn btn-default\" ng-if=\"uiRouterAvailable === true\" ui-sref={{detailRoute}}(row[identityValue])><i class=\"glyphicon glyphicon-info-sign\"></i></button> <button type=button class=\"btn btn-default\" ng-if=\"uiRouterAvailable === false\" ng-click=open(row[identityValue]);><i class=\"glyphicon glyphicon-info-sign\"></i></button> <button type=button class=\"btn btn-default\" ng-click=delete([row[identityValue]]);><i class=\"glyphicon glyphicon-remove\"></i></button></div></div></div></li></ul><nav ng-if=\"pages > 1\"><ul class=pagination><li ng-click=setPreviousPage();><span aria-hidden=true>&laquo;</span></li><li ng-repeat=\"n in [] | range:pages\" ng-click=setPage(n+1);><span>{{n+1}}</span></li><li ng-click=setNextPage();><span aria-hidden=true>&raquo;</span></li></ul></nav></div></div>"
  );
